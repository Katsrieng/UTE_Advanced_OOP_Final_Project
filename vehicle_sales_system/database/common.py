...
def settings():
    ...
    if not re.fullmatch(r'[A-Za-z][A-Za-z0-9_]{0,63}', name):
        raise ValueError('DB_NAME must start with a letter and contain only letters, numbers and underscores (max 64).')
    return dict(host=values.get('DB_HOST', 'localhost'), port=int(values.get('DB_PORT', '3307')),
                user=values.get('DB_USER', 'root'), password=values.get('DB_PASSWORD') or '',
                database=name, charset='utf8mb4', collation='utf8mb4_unicode_ci',
...
def execute_script(connection, path, database=None):
    ...
    sql = '\n'.join(line for line in Path(path).read_text(encoding='utf-8-sig').splitlines()
