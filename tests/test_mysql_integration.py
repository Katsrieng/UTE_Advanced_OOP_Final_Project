...
class MySQLIntegrationTests(MySQLTestCase):
    ...
    def test_sale_rollback_at_invoice_failure(self):
        ...
        def fail(sql,params=()):
            if sql.startswith('INSERT INTO invoices'): raise RuntimeError('injected invoice outage')
            return execute(sql,params)
        with patch.object(repo.db,'execute',side_effect=fail):
            with self.assertRaises(RuntimeError): SalesService(repo).complete(1,1,'50.25',1)
    ...
    def test_seed_repeat_does_not_duplicate_or_restore_removed_grants(self):
        ...
        self.assertEqual(repo.db.query("SELECT COUNT(*) AS n FROM invoices i JOIN sales s ON s.sale_id=i.sale_id WHERE i.total_amount<>s.total_amount")[0]['n'],0)
    ...
    def test_database_outage_is_friendly_503(self):
        ...
        app=create_app(self.app_config())
        with patch.object(app.extensions['repository'],'get',side_effect=DatabaseUnavailable('private details')):
            response=app.test_client().get('/')
        self.assertEqual(response.status_code,503)
        self.assertNotIn('private details',response.text)

    def test_schema_length_validation_preserves_form(self):
        app=create_app(self.app_config()); client=app.test_client(); client.get('/login')
        with client.session_transaction() as session:
            session['user_id']=1; csrf=session['csrf_token']
        response=client.post('/customers/new',data=dict(csrf_token=csrf,code='X'*81,name='Length check',phone='000',email='length@example.com',address='',status='ACTIVE'))
    ...
    def test_seed_rejects_missing_stockout(self):
        from database.seed import seed_database
        repo=create_app(self.app_config()).extensions['repository']
        repo.db.execute("DELETE FROM stock_movements WHERE reason='Sale SALE-DEMO-001'")
        with self.assertRaisesRegex(ValueError,'inconsistent'):
            seed_database(self.db_config)

    def test_seed_user_rename_preserves_identity_passwords_and_grants(self):
        from database.seed import seed_database, DEMO_USERS
        repo=create_app(self.app_config()).extensions['repository']
        before=repo.db.query('SELECT user_id,password_hash FROM users ORDER BY user_id')
        grants=repo.db.query('SELECT user_id,role_id FROM user_roles ORDER BY user_id,role_id')
        for data in DEMO_USERS:
            repo.db.execute('UPDATE users SET username=%s WHERE username=%s',
                            (data['legacy_username'],data['username']))
        seed_database(self.db_config)
        self.assertEqual(before,repo.db.query('SELECT user_id,password_hash FROM users ORDER BY user_id'))
        self.assertEqual(grants,repo.db.query('SELECT user_id,role_id FROM user_roles ORDER BY user_id,role_id'))
        self.assertEqual([u['name'] for u in repo.all('users')],['Katsrieng','Chan Sopanha','Sovanara'])
