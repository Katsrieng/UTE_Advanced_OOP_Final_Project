...
class ApplicationTests(MySQLTestCase):
    ...
    def test_login_and_csrf(self):
        ...
        self.assertIn(
            "incorrect",
            self.post("/login", {"username": "katsrieng", "password": "wrong"}).text,
        )
        self.assertEqual(
            self.post(
    ...
    def test_sales_staff_permissions_enforced_on_server(self):
        ...
        self.assertEqual(
            self.post("/users/new", {"name": "Unauthorized"}).status_code, 403
        )
    ...
    def test_sale_creates_exactly_one_invoice_and_movement(self):
        counts = {
            key: len(self.repository.all(key))
            for key in ("sales", "invoices", "inventory")
        }
        response = self.post(
            "/sales/new", {"customer_id": 1, "vehicle_id": 1, "discount": "900"}
        )
        ...
        duplicate = self.post(
            "/sales/new", {"customer_id": 2, "vehicle_id": 1, "discount": "0"}
        )
    ...
    def test_invalid_sale_does_not_change_records(self):
        for vehicle, customer, discount in [
            (3, 1, "0"),
            (5, 1, "0"),
            (1, 999, "0"),
            (1, 12, "0"),
            ...
            response = self.post(
                "/sales/new",
                {"customer_id": customer, "vehicle_id": vehicle, "discount": discount},
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_stock_movement_transitions(self):
        ...
        duplicate = self.post(
            "/inventory/new",
            {"vehicle_id": "5", "movement": "STOCK_IN", "reason": "Duplicate receipt"},
        )
        self.assertIn("already in stock", duplicate.text)
        self.post(
            "/inventory/new",
            {"vehicle_id": "5", "movement": "STOCK_OUT", "reason": "Supplier return"},
        )
        self.assertEqual(self.repository.get("vehicles", 5)["status"], "INACTIVE")

    def test_filters_pagination_empty_and_errors(self):
        self.assertIn(
            "BYD", self.client.get("/vehicles?brand=BYD&status=AVAILABLE").text
        )
        self.assertIn(
            "No vehicles match", self.client.get("/vehicles?q=no-such-vehicle").text
        )
        self.assertIn("Showing 9", self.client.get("/vehicles?page=2").text)
        self.assertIn(
            "Choose a valid start",
            self.client.get("/reports?period=custom&from=bad&to=bad").text,
        )
        for path in [
            "/missing",
            "/vehicles/999",
