...
class VehiclePhotoTests(MySQLTestCase):
    ...
    def image(self, format='PNG', filename=None):
        ...
        stream.seek(0)
        return stream, filename or ('car.' + {'JPEG': 'jpg', 'PNG': 'png', 'WEBP': 'webp'}[format])

    def fields(self, vehicle=None):
        if vehicle:
            return {key: vehicle[key] for key in ('code', 'brand', 'model', 'year', 'color', 'vin', 'plate', 'purchase_price', 'price', 'status')}
        return dict(code='PHOTO-001', brand='AION', model='Photo Test', year=2026, color='Blue',
                    vin='PHOTO000000000001', plate='', purchase_price=20000, price=25000, status='AVAILABLE')

    def post(self, path, data):
        response = self.client.post(path, data=dict(data, csrf_token=self.token), content_type='multipart/form-data')
    ...
    def upload_existing(self, format='PNG'):
        response = self.post('/vehicles/1/edit', dict(self.fields(self.repo.get('vehicles', 1)), photo=self.image(format)))
    ...
    def test_create_with_each_supported_format(self):
        for i, format in enumerate(('PNG', 'JPEG', 'WEBP')):
            with self.subTest(format=format):
                fields = dict(self.fields(), code=f'PHOTO-{i}', vin=f'PHOTO{i:012}', photo=self.image(format))
                response = self.post('/vehicles/new', fields)
                self.assertEqual(response.status_code, 302)
                vehicle = self.repo.all('vehicles')[-1]
                self.assertRegex(vehicle['image'], r'^uploads/vehicles/vehicle_[a-f0-9]{32}\.(png|jpg|webp)$')
    ...
    def test_invalid_extension_and_disguised_content_rejected(self):
        for upload in (self.image(filename='car.svg'), (BytesIO(b'<script>bad</script>'), 'car.png'), self.image(filename='car.jpg')):
    ...
    def test_edit_without_upload_preserves_photo(self):
        ...
        response = self.post('/vehicles/1/edit', dict(self.fields(self.repo.get('vehicles', 1)), color='Silver'))
        self.assertEqual(response.status_code, 302)
    ...
    def test_removal_requires_confirmation_permission_and_csrf(self):
        ...
        self.assertEqual(self.client.post('/vehicles/1/photo/remove', data={'confirm_remove': 'yes'}).status_code, 403)
        with self.client.session_transaction() as session:
            session['user_id'] = 3
        self.assertEqual(self.post('/vehicles/1/photo/remove', {'confirm_remove': 'yes'}).status_code, 403)
        self.assertTrue((self.root / old).exists())
    ...
    def test_original_filename_cannot_control_path(self):
        response = self.post('/vehicles/new', dict(self.fields(), photo=self.image(filename='../../outside.png')))
        self.assertEqual(response.status_code, 302)
    ...
    def test_invalid_form_keeps_old_photo_and_creates_no_orphans(self):
        old = self.upload_existing()
        response = self.post('/vehicles/1/edit', dict(self.fields(self.repo.get('vehicles', 1)), vin='bad', photo=self.image('WEBP')))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.repo.get('vehicles', 1)['image'], old)
    ...
    def test_repository_failure_keeps_old_photo_and_cleans_new_file(self):
        ...
        with patch.object(self.repo, 'save', side_effect=OSError('storage unavailable')):
            response = self.post('/vehicles/1/edit', dict(self.fields(self.repo.get('vehicles', 1)), photo=self.image('WEBP')))
        self.assertEqual(response.status_code, 200)
        self.assertIn('photo-error', response.text)
        self.assertEqual(self.repo.get('vehicles', 1)['image'], old)
        self.assertEqual(len(self.uploaded_files()), 1)
    ...
    def test_oversized_upload_has_friendly_error_and_keeps_record(self):
        response = self.post('/vehicles/1/edit', dict(self.fields(self.repo.get('vehicles', 1)), photo=(BytesIO(b'x' * (5*1024*1024+1)), 'large.png')))
    ...
    def test_filename_collision_does_not_delete_existing_file(self):
        ...
        with patch('app.services.vehicle_photos.uuid4', return_value=SimpleNamespace(hex='a'*32)):
            response = self.post('/vehicles/new', dict(self.fields(), photo=self.image()))
        self.assertEqual(response.status_code, 200)
