...
class DatabaseSetupTests(unittest.TestCase):
    def test_setup_entrypoint_exists(self):
        self.assertTrue((ROOT / 'database' / 'setup.py').is_file(), 'Missing explicit database setup command')
    ...
    def test_schema_creates_database_before_tables(self):
        ...
        self.assertLess(sql.index('CREATE DATABASE IF NOT EXISTS'), sql.index('CREATE TABLE IF NOT EXISTS'))
...
class SetupSafetyTests(unittest.TestCase):
    def test_connects_without_selecting_nonexistent_database(self):
        ...
        with patch('database.common.mysql.connector.connect', return_value=connection) as connect:
            create_schema(dict(host='localhost',user='tester',password='not-a-real-secret',database='autovault_test_setup'))
        ...
        self.assertIn('CREATE DATABASE IF NOT EXISTS `autovault_test_setup`', statements[0])
        self.assertIn('USE `autovault_test_setup`', statements[1])
        self.assertEqual(sum('CREATE TABLE IF NOT EXISTS' in sql for sql in statements), 10)
    ...
    def test_unsafe_database_identifier_rejected_before_sql(self):
        ...
        with self.assertRaises(ValueError):
            execute_script(connection, ROOT / 'database/schema.sql', 'bad`; DROP DATABASE other;--')
