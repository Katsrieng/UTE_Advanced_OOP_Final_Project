...
class VehiclePhotoService:
    ...
    UPLOAD_PATTERN = re.compile(
        r"uploads/vehicles/vehicle_[0-9a-f]{32}\.(jpg|jpeg|png|webp)\Z"
    )
    ...
    def save_upload(self, upload):
        ...
        if len(data) > self.max_bytes:
            raise PhotoError(
                "The photo is too large. Choose an image no larger than 5 MB."
            )
        image = None
        try:
            with warnings.catch_warnings():
                ...
                with Image.open(BytesIO(data)) as source:
                    if source.format != self.FORMATS[extension]:
                        raise PhotoError(
                            "The photo content does not match its file extension."
                        )
                    if source.width * source.height > self.MAX_PIXELS:
                        raise PhotoError(
                            "Choose a photo with no more than 20 million pixels."
                        )
                    if getattr(source, "n_frames", 1) != 1:
                        raise PhotoError(
                            "Choose a still vehicle photo rather than an animated image."
                        )
                    source.verify()
                with Image.open(BytesIO(data)) as source:
                    source.load()
                    ...
                    image = oriented.convert(
                        "RGB" if extension in (".jpg", ".jpeg") else "RGBA"
                    )
                    oriented.close()
            ...
            if path is None:
                raise PhotoError(
                    "Photo storage is unavailable. Please contact the administrator."
                )
            ...
            raise PhotoError(
                "The photo could not be read or saved. Choose a valid JPG, PNG or WebP image and try again."
            ) from exc
        finally:
    ...
    def remove_unreferenced(self, image):
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "Photo cleanup postponed because reference lookup failed."
            )
            return
        try:
            ...
            logging.getLogger(__name__).warning(
                "Could not clean an unreferenced vehicle photo: %s", image
            )
    ...
    def save_vehicle(self, values, item_id=None, upload=None, remove=False):
        ...
        values = dict(values)
        new_image = (
            self.save_upload(upload) if upload is not None and upload.filename else None
        )
        old_image = "sedan.svg"
        try:
            with self.repository.transaction():
                original = (
                    self.repository.get("vehicles", item_id, lock=True)
                    if item_id
                    else None
                )
                ...
                old_image = (
                    original.get("image", "sedan.svg") if original else "sedan.svg"
                )
                if original and original["status"] == "SOLD":
                    ...
                    if any(
                        key in values and values[key] != original[key]
                        for key in protected
                    ):
                        raise ValueError(
                            "Sold vehicle identity and price are preserved for invoice history."
                        )
            ...
            message = (
                str(exc)
                if isinstance(exc, (PersistenceError, ValueError))
                else "The vehicle could not be saved. Your previous photo has been kept. Please try again."
            )
